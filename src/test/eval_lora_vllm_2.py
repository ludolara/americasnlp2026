...
from test.eval_lora import (
    ...
    load_tokenizer as load_transformers_tokenizer,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run SacreBLEU evaluation for Aya Vision merged checkpoints with vLLM "
            "using tokenizer and decoding settings aligned with the transformers evaluator."
        )
    )
    parser.add_argument(
        "--model-name-or-path",
    ...
    parser.add_argument(
        ...
        default=1.0,
        help=(
            "Sampling temperature shared with the transformers backend when "
            "--generation-budget is greater than 1."
        ),
    )
    parser.add_argument(
        "--top-k",
        type=int,
        default=50,
        help="Top-k sampling constraint shared with the transformers backend.",
    )
    parser.add_argument(
        "--top-p",
        type=float,
        default=None,
        help="Optional top-p sampling constraint shared with the transformers backend.",
    )
    parser.add_argument(
...
def _load_tokenizer(model_name_or_path: str):
    return load_transformers_tokenizer(model_name_or_path)
...
def _build_run_dir(args: argparse.Namespace) -> Path:
    ...
    top_p_slug = _slugify_path_component(
        "none" if args.top_p is None else str(args.top_p).replace(".", "p")
    )
    run_name = (
        f"{model_slug}__{dataset_slug}__{args.split}"
        f"__langs-{language_slug}__vllm-2"
        f"__temp-{temperature_slug}__topk-{top_k_slug}__topp-{top_p_slug}"
...
def _build_sampling_params(args: argparse.Namespace, tokenizer: Any):
    ...
    params: dict[str, Any] = {"max_tokens": args.max_new_tokens}
    ...
    *,
    ...
    _write_json(
        run_dir / "summary.json",
        {
            ...
            "decoding_mode": "greedy" if args.generation_budget == 1 else "sampling",
            "temperature": args.temperature,
            "top_k": args.top_k,
            "top_p": args.top_p,
            "dtype": args.dtype,
        ...
        + json.dumps(
            {
                ...
                "score_budgets": args.score_budgets,
                "decoding_mode": (
                    "greedy" if args.generation_budget == 1 else "sampling"
                ),
                "temperature": args.temperature,
                "top_k": args.top_k,
                "top_p": args.top_p,
